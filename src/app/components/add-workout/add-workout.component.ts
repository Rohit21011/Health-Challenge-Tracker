...
export class AddWorkoutComponent {
  ...
  addWorkout(value:workoutForm) {
    ...
    const existingUser = this.users.find(user => user.name.toLowerCase() === value.name.toLowerCase());

    if (existingUser) {
      const existingWorkout = existingUser.workouts.find(workout => workout.type === value.types);
      existingWorkout?existingWorkout.minutes +=value.minutes: existingUser.workouts.push({ type: value.types, minutes: value.minutes });
    ...
    (Object as any).values(this.workoutForm.controls).forEach((control: FormControl) => {
